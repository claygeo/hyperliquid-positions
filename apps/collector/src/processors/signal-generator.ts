// Signal Generator V12.1
// CHANGES FROM V12:
// - NEW: Fresh position filtering - only include traders who opened within MAX_POSITION_AGE_HOURS
// - Prevents signals from being created for stale positions opened days/weeks ago
// - Ensures signals represent fresh alpha, not old holdings
//
// CHANGES FROM V11:
...
// V12.1: Updated to filter out stale positions
async function getAllTradersInPosition(
  ...
  for (const pos of positions) {
    // V12.1: Filter out stale positions
...
async function checkPriceTargets(): Promise<void> {
  ...
  for (const signal of activeSignals) {
    ...
    let hitStop = direction === 'long' ? currentPrice <= signal.stop_loss : currentPrice >= signal.stop_loss;
    let hitTp1 = signal.hit_tp1 || (direction === 'long' ? currentPrice >= signal.take_profit_1 : currentPrice <= signal.take_profit_1);
    let hitTp2 = signal.hit_tp2 || (direction === 'long' ? currentPrice >= signal.take_profit_2 : currentPrice <= signal.take_profit_2);
    let hitTp3 = signal.hit_tp3 || (direction === 'long' ? currentPrice >= signal.take_profit_3 : currentPrice <= signal.take_profit_3);
    ...
    if (hitStop) {
      ...
      const tradersWithExit = (signal.traders || []).map((trader: TraderForSignal) => ({
        ...trader, exit_price: trader.exit_price ?? currentPrice, exited_at: trader.exited_at ?? closedAt, exit_type: trader.exit_type ?? 'stopped' as ExitType,
      }));

      await db.client.from('quality_signals').update({
        is_active: false, outcome: 'stopped_out', hit_stop: true, final_pnl_pct: currentPnlPct,
        current_price: currentPrice, closed_at: closedAt, traders: tradersWithExit,
      }).eq('id', signal.id);
      ...
      const closedAt = new Date().toISOString();
      const tradersWithExit = (signal.traders || []).map((trader: TraderForSignal) => ({
        ...trader, exit_price: trader.exit_price ?? currentPrice, exited_at: trader.exited_at ?? closedAt, exit_type: trader.exit_type ?? 'manual' as ExitType,
      }));

      await db.client.from('quality_signals').update({
        is_active: false, outcome: 'tp3_hit', hit_tp1: true, hit_tp2: true, hit_tp3: true,
        final_pnl_pct: currentPnlPct, current_price: currentPrice, closed_at: closedAt, traders: tradersWithExit,
      }).eq('id', signal.id);

      logger.info(`🎯🎯🎯 TP3 HIT: ${signal.coin} ${signal.direction} | P&L: +${currentPnlPct.toFixed(2)}%`);
    } else {
      await db.client.from('quality_signals').update({
        current_price: currentPrice, current_pnl_pct: currentPnlPct,
        peak_price: peakPrice, trough_price: troughPrice,
        max_pnl_pct: Math.max(signal.max_pnl_pct || 0, currentPnlPct),
        min_pnl_pct: Math.min(signal.min_pnl_pct || 0, currentPnlPct),
        hit_tp1: hitTp1, hit_tp2: hitTp2, updated_at: new Date().toISOString(),
      }).eq('id', signal.id);
    }
  }
}
...
export function initializeSignalGenerator(): void {
  logger.info(`Signal Generator V12.1 initializing (fresh positions only: ${MAX_POSITION_AGE_HOURS}h max age)...`);
  ...
  logger.info('Signal Generator V12.1 ready - filters stale positions, tracks per-trader exit data');
}
...
export function stopSignalGenerator(): void {
  ...
  logger.info('Signal Generator V12.1 stopped');
}
...
export async function generateSignals(): Promise<void> {
  logger.debug('generateSignals() - V12.1 is event-driven');
}
